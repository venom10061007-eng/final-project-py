...
# مسار الـ Desktop تلقائياً بغض النظر عن اسم المستخدم
DESKTOP = os.path.join(os.path.expanduser("~"), "Desktop")

raw_path = os.path.join(DESKTOP, "AmesHousing.csv")
...
# Collect all duplicate rows then delete them all
duplicates = df[df.duplicated(keep=False)]
df = df.drop_duplicates()
print(f'Removed {len(duplicates)} duplicate rows. Rows remaining: {len(df)}')
...
# Save cleaned CSV to Desktop for Phase 2
cleaned_path = os.path.join(DESKTOP, "AmesHousing_cleaned.csv")
